...
export const PUBLIC_EXAM_YEAR_WINDOW = 10;

export function getPublicExamYearRange(referenceDate = new Date()): { minYear: number; maxYear: number } {
  const maxYear = referenceDate.getFullYear();
  return { minYear: maxYear - (PUBLIC_EXAM_YEAR_WINDOW - 1), maxYear };
}

export function isRecentPublicExamYear(year: number | undefined, referenceDate = new Date()): boolean {
...
export function discoverShenlunCandidatesFromLinks(
  ...
  const referenceDate = new Date(discoveredAt);
  ...
  for (const link of links) {
    ...
    if (!isRecentPublicExamYear(year, safeReferenceDate)) continue;
