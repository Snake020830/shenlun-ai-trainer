...
function PublicExamBrowser({ onImported }: { onImported: () => Promise<void> | void }) {
  ...
  useEffect(() => {
    void reload().catch(error => {
      ...
      setStatus(errorMessage(error, "无法读取本机公开真题目录。"));
    });
  }, []);
  ...
  async function autoCompleteLibrary() {
    ...
    try {
      const result = await initializeRecentPublicExamLibrary({
        delayMs: 350,
        onProgress: progress => setStatus(bootstrapStatus(progress))
      });
      await reload();
      setStatus(
        `自动补全完成：已入库 ${result.finalImportedPaperCount}/${result.candidateCount} 套；本轮新增 ${result.import.questionCount} 道题。` +
        `${result.audit.blocked + result.audit.error + result.import.error > 0 ? ` 另有 ${result.audit.blocked + result.audit.error + result.import.error} 套异常来源已隔离，不影响刷题。` : ""}`
      );
      await onImported();
    } catch (error) {
  ...
  async function scanPrimaryCatalog() {
    ...
    try {
      ...
      setStatus(errorMessage(error, "公开真题目录更新失败。"));
    } finally {
      setBusy(null);
    }
  }
  ...
  async function openPreview(candidate: PublicSourceCandidate) {
    ...
    try {
      ...
      setStatus(canImportParsedPublicExam(next.exam)
        ? `已识别 ${next.exam.materials.length} 则材料、${next.exam.tasks.length} 道题。这里的人工核验只用于你主动检查异常/特定试卷，不是日常导入必需步骤。`
        : "这套卷仍有未解决的解析警告，当前不会写入正式题库。"
      );
    } catch (error) {
      console.error("Failed to preview public exam.", error);
      const message = errorMessage(error, "整卷预览失败。");
      setPreviewError({ candidateId: candidate.id, message });
      setStatus(`预览失败：${candidate.title}。${message}`);
    } finally {
      setBusy(null);
    }
  }
  ...
  async function importExam() {
    ...
    try {
      ...
      setStatus(errorMessage(error, "整卷导入失败。"));
    } finally {
      setBusy(null);
    }
  }
  ...
  return <div className="public-library-browser">
    <div className="public-library-intro public-library-auto-intro">
      <div><Globe2 size={20}/><div><strong>公开申论整卷 · 近10年</strong><span>正常试卷由程序自动校验后入库；只有解析警告/来源异常的试卷会被隔离。你无需逐卷人工核对。</span></div></div>
      <div className="public-library-auto-actions">
        <button className="primary" disabled={!desktop || busy !== null} onClick={() => void autoCompleteLibrary()}><Download size={15}/>{busy === "bootstrap" ? "自动补全中…" : "自动补全题库"}</button>
        <button className="secondary" disabled={!desktop || busy !== null} onClick={() => void scanPrimaryCatalog()}><RefreshCw size={15}/>{busy === "scan" ? "更新中…" : candidates.length ? "只更新目录" : "获取公开目录"}</button>
      </div>
    </div>
      ...
      {rows.map(group => {
        ...
        return <article className="public-exam-card" key={group.key}>
          <div className="public-exam-card-heading">
            ...
            <button disabled={!desktop || busy !== null || group.hasImportedVersion} onClick={() => void openPreview(item)}><Eye size={14}/>{busy === item.id ? "读取中" : group.hasImportedVersion ? "已有版本入库" : "手动检查"}</button>
          </div>
          {expanded && <div className="public-exam-variants">{group.alternatives.map(alternative => <div key={alternative.id}><div><strong>{alternative.title}</strong><span>{sourceVariantLabel(alternative)}</span></div><div>{!group.hasImportedVersion && <button disabled={!desktop || busy !== null} onClick={() => void openPreview(alternative)}><Eye size={13}/>{busy === alternative.id ? "读取中" : "检查此版本"}</button>}<a href={alternative.sourceUrl} target="_blank" rel="noreferrer"><ExternalLink size={13}/>原页面</a></div></div>)}</div>}
        </article>;
      })}
      {!rows.length && <div className="public-library-empty">{candidates.length ? "没有符合筛选条件的整卷。" : `尚未获取 ${yearRange.minYear}—${yearRange.maxYear} 公开整卷目录。桌面版点击“自动补全题库”即可开始。`}</div>}
    </div>
...
export default function QuestionLibraryPage({
  ...
  useEffect(() => {
    void getQuestionNoteIds().then(setNoteIds).catch(error => console.error("Failed to load question note ids.", error));
  }, [allQuestions.length]);

  const typeCounts = useMemo(() => {
    const counts = new Map<QuestionType, number>(QUESTION_TYPES.map(type => [type, 0]));
    for (const question of allQuestions) counts.set(question.type, (counts.get(question.type) ?? 0) + 1);
    return counts;
  }, [allQuestions]);

  const filterOptions = useMemo(() => ({
    years: [...new Set(allQuestions.map(question => question.year))].sort((a, b) => b - a),
    regions: [...new Set(allQuestions.map(question => question.region).filter(Boolean))].sort((a, b) => a.localeCompare(b, "zh-CN")),
    difficulties: [...new Set(allQuestions.map(question => question.difficulty))]
  }), [allQuestions]);
  ...
  const filtered = useMemo(() => {
    ...
    return allQuestions
      ...
      .filter(question => !needle || `${question.title}${question.type}${question.tags.join("")}${question.region}`.toLowerCase().includes(needle))
      .sort((left, right) => right.year - left.year || left.title.localeCompare(right.title, "zh-CN"));
  ...
  async function openNote(question: Question) {
    ...
    try {
      const note = await getQuestionNote(question.id);
      setNoteEditor(current => current?.question.id === question.id
        ? { ...current, value: note?.content ?? "", loading: false }
        : current
      );
    } catch (error) {
      console.error("Failed to load question note.", error);
      setNoteEditor(current => current?.question.id === question.id ? { ...current, loading: false } : current);
    }
  }

  async function persistNote() {
    if (!noteEditor || noteEditor.saving) return;
    const questionId = noteEditor.question.id;
    const content = noteEditor.value;
    setNoteEditor(current => current ? { ...current, saving: true } : current);
    try {
      await saveQuestionNote(questionId, content);
      setNoteIds(current => {
        const next = new Set(current);
        if (content.trim()) next.add(questionId); else next.delete(questionId);
        return next;
      });
      setNoteEditor(null);
    } catch (error) {
      console.error("Failed to save question note.", error);
      setNoteEditor(current => current ? { ...current, saving: false } : current);
    }
  }

  function startRandomFilteredQuestion() {
    if (!filtered.length) return;
    onStart(filtered[Math.floor(Math.random() * filtered.length)]);
  }

  return <main className="page page-wide question-library-page training-library-v2">
    <header className="page-header compact"><div><p className="eyebrow">题库</p><h1>按题型刷真题</h1><p>先选专项，再按年份、地区或难度缩小范围。公开题库可自动补全；每道题都有独立私人笔记。</p></div><button className="secondary" onClick={onImport}><Plus size={16}/>手工导入</button></header>
    ...
    {tab === "ready" ? <>
      <section className="training-type-panel">
        <div className="training-type-heading"><div><strong>专项训练</strong><span>点击题型直接筛选</span></div><button className="secondary" disabled={!filtered.length} onClick={startRandomFilteredQuestion}>随机一题</button></div>
        <div className="training-type-grid">
          <button className={typeFilter === "all" ? "active" : ""} onClick={() => setTypeFilter("all")}><strong>全部题型</strong><span>{allQuestions.length} 题</span></button>
          {QUESTION_TYPES.map(type => <button key={type} className={typeFilter === type ? "active" : ""} onClick={() => setTypeFilter(type)}><strong>{type}</strong><span>{typeCounts.get(type) ?? 0} 题</span></button>)}
        </div>
      </section>

      <div className="training-filter-bar">
        <div className="search-box"><Search size={17}/><input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索题目或关键词"/></div>
        <select value={yearFilter} onChange={event => setYearFilter(event.target.value)}><option value="all">全部年份</option>{filterOptions.years.map(year => <option key={year} value={year}>{year}</option>)}</select>
        <select value={regionFilter} onChange={event => setRegionFilter(event.target.value)}><option value="all">全部地区</option>{filterOptions.regions.map(region => <option key={region} value={region}>{region}</option>)}</select>
        <select value={difficultyFilter} onChange={event => setDifficultyFilter(event.target.value)}><option value="all">全部难度</option>{filterOptions.difficulties.map(value => <option key={value} value={value}>{value}</option>)}</select>
        <button className="text-button" onClick={() => { setQuery(""); setYearFilter("all"); setRegionFilter("all"); setDifficultyFilter("all"); }}>清除细筛</button>
        <span className="library-count">{filtered.length} 题</span>
      </div>
      ...
      <div className="question-grid">{filtered.map(question => {
        ...
        const hasNote = noteIds.has(question.id);
        return <article className="question-card" key={question.id}>
          ...
          <div className="question-card-tools">
            <button type="button" className={hasNote ? "has-note" : ""} onClick={() => void openNote(question)}><FileText size={13}/>{hasNote ? "查看笔记" : "写笔记"}</button>
            {publicQuestion && <button type="button" onClick={() => void toggleSource(question.id)} disabled={sourceLoadingId !== null}><Info size={13}/>{sourceLoadingId === question.id ? "读取来源…" : detail ? "收起来源" : "来源"}</button>}
            {question.tags.includes("回忆版") && <span>回忆版</span>}
          </div>
    ...
    {noteEditor && <div className="question-note-backdrop" role="dialog" aria-modal="true" aria-label="我的题目笔记">
      <div className="question-note-dialog">
        <header><div><span>我的笔记</span><strong>{noteEditor.question.title}</strong><small>{noteEditor.question.type} · {noteEditor.question.year} · {noteEditor.question.region}</small></div><button className="text-button" disabled={noteEditor.saving} onClick={() => setNoteEditor(null)}>关闭</button></header>
        <textarea
          value={noteEditor.value}
          onChange={event => setNoteEditor(current => current ? { ...current, value: event.target.value } : current)}
          disabled={noteEditor.loading || noteEditor.saving}
          placeholder={noteEditor.loading ? "正在读取笔记…" : "记录自己的审题思路、踩坑点、得分词、重做提醒……"}
        />
        <footer><span>{noteEditor.value.length} 字 · 仅保存在你的本地题库数据中</span><button className="primary" disabled={noteEditor.loading || noteEditor.saving} onClick={() => void persistNote()}>{noteEditor.saving ? "保存中…" : "保存笔记"}</button></footer>
      </div>
    </div>}
  </main>;
}
