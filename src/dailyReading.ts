...
export type DailyArticleScope = "central" | "anhui";
...
export const DAILY_READING_THEMES: DailyReadingTheme[] = [
  ...
  { id: "safety-risk", name: "公共安全与风险治理", focus: "食品药品、安全生产、应急处置与源头预防", keywords: ["公共安全", "食品安全", "药品安全", "安全生产", "应急", "风险隐患", "源头治理"], readingPrompt: "按“排查—预警—处置—复盘”提炼风险治理闭环。" },
...
export const DAILY_NEWS_PROVIDERS: DailyNewsProvider[] = [
  { id: "anhui-central", name: "安徽省政府网·中央要闻", indexUrl: "https://www.ah.gov.cn/zwyw/dzy/index.html", scope: "central" },
  { id: "anhui-local", name: "安徽省政府网·安徽要闻", indexUrl: "https://www.ah.gov.cn/zwyw/jryw/index.html", scope: "anhui" },
  { id: "anhui-focus", name: "安徽省政府网·政策与专题", indexUrl: "https://www.ah.gov.cn/zwyw/index.html", scope: "anhui" }
];
...
function linksFromIndex(provider: DailyNewsProvider, html: string, fetchedUrl: string): DailyNewsCandidate[] {
  ...
  for (const anchor of document.querySelectorAll<HTMLAnchorElement>("a[href]")) {
    ...
    if (url.protocol !== "https:" || !/(^|\.)ah\.gov\.cn$/i.test(url.hostname) || !/\/\d+\.html$/i.test(url.pathname)) continue;
    url.hash = "";
...
function extractArticle(candidate: DailyNewsCandidate, html: string): DailyNewsArticle | null {
  const document = new DOMParser().parseFromString(html, "text/html");
  const contentRoot = document.querySelector(".wzcon, .article-content, .pages_content, article, [role='article']");
  ...
  const title = metaContent(document, "ArticleTitle") || candidate.title;
  const source = metaContent(document, "ContentSource") || candidate.provider.name;
  const publishedAt = (metaContent(document, "PubDate") || candidate.dateHint || "").slice(0, 10);
...
export function scoreDailyArticle(article: DailyNewsArticle, theme: DailyReadingTheme, referenceDate = new Date()): number {
  ...
  const ageDays = published && !Number.isNaN(published.getTime()) ? Math.max(0, (referenceDate.getTime() - published.getTime()) / 86_400_000) : 30;
  const recency = ageDays <= 3 ? 20 : ageDays <= 14 ? 17 : ageDays <= 45 ? 12 : ageDays <= 120 ? 6 : 0;
  ...
  return Math.max(0, Math.min(100, 25 + recency + Math.min(22, themeHits * 6) + Math.min(13, adminHits * 2) + Math.min(10, structureHits * 2) + lengthScore - titlePenalty));
}
...
function preliminaryScore(candidate: DailyNewsCandidate, theme: DailyReadingTheme): number {
  return countKeywordHits(candidate.title, theme.keywords) * 8
    ...
    + (candidate.provider.scope === "anhui" ? 2 : 0)
    - (LOW_VALUE_TITLE.test(candidate.title) ? 12 : 0);
}

function reasonFor(article: DailyNewsArticle, theme: DailyReadingTheme): string {
  const role = article.role === "policy" ? "制度依据和治理框架完整" : "有具体主体、动作和治理结果";
  const scope = article.scope === "anhui" ? "补充安徽本地表达与案例" : "建立全国政策视野";
  return `${role}；${scope}，可训练“${theme.focus}”。`;
}

function choosePair(articles: DailyNewsArticle[]): DailyNewsArticle[] {
  const sorted = [...articles].sort((a, b) => b.score - a.score);
  const first = sorted.find(article => article.role === "policy" && article.scope === "central")
    ?? sorted.find(article => article.role === "policy")
    ?? sorted[0];
  const remaining = sorted.filter(article => article.id !== first?.id);
  const second = remaining.find(article => article.role === "case" && article.scope === "anhui")
    ?? remaining.find(article => article.scope !== first?.scope)
    ?? remaining.find(article => article.role !== first?.role)
    ?? remaining[0];
  return [first, second].filter((article): article is DailyNewsArticle => Boolean(article));
}
...
export async function acquireDailyReadingPlan(
  ...
  const articles = fetched.filter((article): article is DailyNewsArticle => Boolean(article)).map(article => {
    ...
    return { ...article, score, selectionReason: reasonFor(article, theme) };
  }).filter(article => article.score >= 45);
  const pair = choosePair(articles);
  if (pair.length < 2) throw new Error("今天通过质量门槛的文章不足两篇。系统没有用低质量会议稿凑数，请稍后刷新。");
  return {
