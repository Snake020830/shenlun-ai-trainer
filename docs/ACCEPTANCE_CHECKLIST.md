...
- [ ] 内置短题进入作答页时明确提示“功能演示题”；不应把 demo 材料误认为正式完整真题。

## C. 实战作答页

### C1. 材料阅读

- [ ] 作答页保持三栏：材料 / 题干与答题纸 / 批改。
- [ ] 材料区不是刺眼纯白后台面板，应呈现低对比暖白纸张感。
- [ ] 材料正文使用适合中文长文阅读的宋体/衬线字体，字重不过粗。
- [ ] 行距约 2 倍、段落首行缩进明显，连续阅读 10 分钟不会觉得拥挤。
- [ ] 每则材料显示编号和大致字数。
- [ ] 粘贴完整长材料后，材料区独立滚动，不把整页或答题区撑坏。

### C2. 原文标记

- [ ] 点击“记号笔”后，拖选原文能增加半透明背景高亮。
- [ ] 点击“下划线”后，拖选原文能增加清晰但不过重的下划线。
- [ ] 标记后正文不跳动、不改变原文字序。
- [ ] “清除标记”能清空当前题的文本标记。
- [ ] 退出题目再重新进入，高亮/下划线仍然存在。
- [ ] 材料标记不会进入 AI 的原材料正文，不应污染评分输入。

> 当前 V0.1 先完成文本高亮/下划线。自由画笔、半透明手绘记号笔和橡皮擦需要建立可随材料滚动/缩放稳定保存的坐标笔迹层后再启用，不能用会错位的临时 canvas 冒充完成。
...
- [ ] 作答区表现为申论答题纸风格，而不是普通空白 textarea。
- [ ] 100% 系统缩放下，中文输入应基本保持“一字一格”的视觉节奏。
- [ ] 网格线足够淡，不压过文字；文字颜色接近纸面书写而不是纯黑粗体。
